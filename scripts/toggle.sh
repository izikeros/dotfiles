...
toggle_redshift()
{
    ...
    while true; do
        if [[ $(pidof redshift) ]]; then
            temp=$(sed 's/K//g' <<< "$(grep -o '[0-9].*K' <<< "$(redshift -p 2 -c ~/redshiftgrc>/dev/null)")")
        fi
        if [[ -z $temp ]]; then
            echo " $icon "                # Greyed out (not running)
        elif [[ $temp -ge 5000 ]]; then
            echo "%{F#8039A0} $icon $temp"     # Blue
        elif [[ $temp -ge 4000 ]]; then
            echo "%{F#F203F0} $icon $temp"     # Yellow
        else
            echo "%{F#FF5B6C} $icon $temp"     # Orange
        fi
        sleep 2
    done
}
